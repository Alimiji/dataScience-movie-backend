# **CineData Insights : Construire un Écosystème de Données Robuste pour l'Industrie Cinématographique**
...
Je publie le SDK (alimiji_film_sdk)  sur [PyPI](https://pypi.org/) permet à n’importe quel utilisateur Python d’installer ton package avec un simple `pip install`. Voici comment faire, étape par étape.
...
Une fois connecté à ton compte PyPI je suis les étapes suivantes:

- Aller dans [Paramètres de compte PyPI](https://pypi.org/manage/account/)
- Clique sur **"Ajouter un jeton d’API"**
- Je lui donne un nom (ex. : `token-sdk-movies`)
...
Dans un **nouveau terminal**, je configure le token pour que l’outil `twine` puisse l’utiliser automatiquement.
...
En testant sur un shell Python :
...
[https://pypi.org/project/alimiji-film-sdk/0.0.5/](https://pypi.org/project/alimiji-film-sdk/0.0.5/)
...
Je viens d'achever ainsi la **Phase 1** de votre parcours vers la maîtrise du développement backend en Python, avec un focus sur l’architecture d’API modernes. Cette étape vous a permis de poser des fondations solides en conception de bases de données, en programmation orientée objet avec SQLAlchemy, en création d’API avec FastAPI, et en publication de SDK sur PyPI.

---

### J'ai conçu la base de cette application de A à Z
...
### J'ai mis en place la couche Data & ORM en Python
...
### J'ai développé une API robuste avec FastAPI
...
### J'ai produit un SDK Python pour interagir avec l’API
...
Dans la **Phase 2 (Data Analyst)**, je vais changer de perspective.

c'est la phase de **consommation des APIs** créées, l'extraction et l' analyse des données à l’aide de pandas, créer des visualisations et des tableaux de bord pour répondre à des questions métiers concrètes. Je vous présenterai  comment utiliser les données publiées par une API (comme la vôtre) pour générer des **insights puissants**.
